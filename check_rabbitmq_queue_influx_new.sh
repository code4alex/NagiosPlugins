...
influx -host 127.0.0.1 -username telegraf -password 123456 -database telegraf -format csv \
-execute 'SELECT mean("messages") FROM "rabbitmq_queue" WHERE "host" =~ /prod-genie-mq1/ AND time > "now() -1m" GROUP BY time(1m), "queue", "node" fill(none)'|\
grep -Ev '^name'|\
